...
export default function App() {
  ...
  const calculateETB = () => {
    if (creatures.some((c) => c.name === "Vrestin")) return;
    ...
    const newCreatures = [
      { name: "Vrestin", counters: vrestinCounters, basePower: vrestinCounters },
      ...Array(base).fill().map((_, i) => ({ name: `Insect ${i + 1}`, counters: insectCounters, basePower: 1 }))
    ];
  ...
  const handleEndStep = () => {
    ...
    if (has("hornbeetle") && counterTracker > 0) {
      ...
      const newTokens = Array(counterTracker).fill().map((_, i) => ({ name: `Beetle Token ${i + 1}`, counters: beetleBonus, basePower: 1 }));
  ...
  const addCreature = () => {
    ...
    addToTracker(final);
    setCreatures([...creatures, { name: newCreatureName, counters: final, basePower: baseCounters }]);
    setNewCreatureName("");
    ...
    <div style={{ padding: "1rem", maxWidth: "1000px", margin: "auto", fontFamily: "sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
            {supportCards.map((card) => (
              <div
                ...
                style={{ cursor: "pointer", padding: "0.5rem", borderRadius: "6px", background: selectedCards.includes(card.id) ? "#cceacc" : "#eee", border: "1px solid #888" }}
              >
                {card.name}
              </div>
            ))}
          ...
          <h2 style={{ marginTop: "1rem" }}>Combat</h2>
          <button onClick={handleCombat} style={{ width: "100%" }}>Attack with Insects</button>

          <h2 style={{ marginTop: "1rem" }}>End Step</h2>
          <button onClick={handleEndStep} style={{ width: "100%" }}>Go to End Step</button>
        </div>

        <div>
          <h2>Creatures</h2>
          <button onClick={clearAllCreatures} style={{ marginBottom: "0.5rem", width: "100%", background: "#500", color: "#fff" }}>❌ Clear All Creatures</button>
          {creatures.map((c, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.3rem", background: "#f4f4f4", padding: "0.3rem 0.5rem", borderRadius: "4px" }}>
              <span>{c.name}: {c.basePower}/{c.basePower} +{c.counters}</span>
              <div>
                <button onClick={() => updateCounter(i, 1)}>+1</button>
                <button onClick={() => updateCounter(i, -1)} style={{ marginLeft: "0.3rem" }}>-1</button>
                <button onClick={() => removeCreature(i)} style={{ marginLeft: "0.3rem", color: "red" }}>🗑️</button>
              </div>
            </div>
          ))}
